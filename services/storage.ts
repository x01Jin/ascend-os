...
export const loadGame = (mode: SaveMode): GameState | null => {
  try {
    ...
    if (dataToLoad) {
      ...
      if (!parsed.stats) {
        parsed.stats = { ...INITIAL_GAME_STATE.stats };
      } else {
        parsed.stats = { ...INITIAL_GAME_STATE.stats, ...parsed.stats };
      }
      if (parsed.secretsZipSeen === undefined) {
        parsed.secretsZipSeen = false;
      }
      if (parsed.hasSeenThankYou === undefined) {
        parsed.hasSeenThankYou = false;
      }
      ...
      if (parsed.fuelPaidIter === undefined) {
        parsed.fuelPaidIter = 0;
      }
