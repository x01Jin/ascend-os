      ...
      <div className="flex-1 overflow-y-auto p-6 space-y-6">
        <div className="bg-gray-900/50 border border-gray-700 rounded-lg p-4 relative overflow-hidden group">
          ...
          <p className="text-xs text-gray-500 mb-4">
            Permanently increases data yield per mining operation.
          </p>
            ...
            <div className="grid grid-cols-2 gap-3">
              ...
              >
                ...
                <span className="text-xs text-red-400">
                  {formatSize(getBoostCost(selectedMultiplier, 5))}
                </span>
              </button>
              <button
              ...
              >
                ...
                <span className="text-xs text-red-400">
                  {formatSize(getBoostCost(selectedMultiplier, 10))}
                </span>
              </button>
            </div>
          </div>
        ...
        <div className="bg-gray-900/50 border border-gray-700 rounded-lg p-4 relative overflow-hidden group">
          ...
          <div className="flex justify-between items-start">
            <div>
              ...
              <p className="text-xs text-gray-500">
                Automatically marks folders as visited when you enter them.
              </p>
            </div>
          ...
          <div className="flex items-center justify-between mt-4">
            <div className="font-mono text-blue-400">{formatSize(autoMarkCost)}</div>
            <button
        ...
        <div className="flex items-start gap-2 p-3 rounded bg-orange-900/10 border border-orange-500/20">
          ...
          <p className="text-xs text-orange-200/80">
            Warning: Efficiency Levels and Upgrade Inventory persist through Ascension. Overclock
            time banks are volatile and may reset on system failure.
          </p>
        </div>
        ...
        <div className="bg-gray-900/50 border border-purple-900 rounded-lg p-4">
          ...
          <p className="text-xs text-gray-500 mb-4">
            One-time purchases. Tools persist through Ascension.
          </p>
          <div className="space-y-3">
            {TOOLS.map(t => {
              const owned = gameState.unlockedTools.includes(t.id);
              const iterOk = gameState.currentIteration >= t.minIter;
              const afford = gameState.dataKB >= t.cost;
              const canBuy = !owned && iterOk && afford;
              return (
                <div
                  key={t.id}
                  className="flex items-center justify-between bg-black/20 p-3 rounded border border-gray-800"
                >
                  <div>
                  ...
                  <button
                    ...
                    className={`
                                    ...
                                    `}
                  >
                    {owned ? 'OWNED' : !iterOk ? `ITER ${t.minIter}+` : 'UNLOCK'}
                  </button>
                </div>
              );
            })}
