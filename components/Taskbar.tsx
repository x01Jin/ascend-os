  ...
  const handleWindowContextMenu = (e: React.MouseEvent, win: WindowState) => {
    ...
    onContextMenu(e.clientX, e.clientY, [
      ...
      { separator: true, label: '' },
      {
        ...
        action: () => (win.isMinimized ? onFocusWindow(win.id) : onMinimize(win.id)),
      },
      {
        label: win.isMaximized ? 'Restore' : 'Maximize',
        action: () => onToggleMaximize(win.id),
      },
      {
        label: 'Close Window',
      ...
      >
        ...
        <div className="flex-1 flex items-center gap-1 overflow-x-auto no-scrollbar">
          {windows.map(win => (
            ...
            >
              <AppWindow
                size={14}
                className={activeWindowId === win.id ? 'text-blue-400' : 'text-gray-500'}
              />
        ...
        <div className="flex items-center gap-3 px-3 text-xs text-gray-400 font-mono border-l border-white/10 pl-4">
          ...
          >
            ...
            <span className="w-16 h-1.5 rounded bg-gray-700 overflow-hidden inline-block">
              <span
                className="block h-full bg-gradient-to-r from-cyan-500 to-purple-500"
