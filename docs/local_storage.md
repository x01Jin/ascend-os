...
Upgrades and inventory: `efficiencyLevel`, `autoMinerData`, `autoMinerInterval`, `autoMarkCount`, `isAutoMarkEnabled`, `boostBank` (ms per x2-x5 multiplier), `activeBoostMultiplier`.

File system: `consumedIds` (opened packages and modules, blocks reload farming), `modifiedNodes` (renames, marks, traces).

Progression: `achievements`, `secretsFound`, `loreSeen`, `secretsZipSeen`, `hasSeenThankYou`, `stats`, `arcadeWins`, `passes`, `fuelPaidIter`, `unlockedTools`.

Flags: `isDevModeEnabled`, `isAscendRootEnabled`. Normal-mode loads force both off.

## Ascension reset

Keeps currency, upgrades, inventory, desktop, seed, and progression (including `arcadeWins`, `passes`, `fuelPaidIter`, and tool unlocks). Clears `modifiedNodes` and `consumedIds`, nulls `activeBoostMultiplier` while banked time stays. The fuel-paid flag and cabinet wins apply per iteration, so a fresh iteration starts unpaid and unwon.
